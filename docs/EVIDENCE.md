...
sigwood has ten detectors, but it does not have ten equally mature bodies of
...
| `protocol` | measured; conclusion not yet published | Four development weeks and two initial held-back weeks from one estate; the expected-port leg, newly unlabeled leg, and a rejected session-shape comparison were measured. | The expected-port and newly unlabeled metadata legs ship opt-in. The held-back session-shape comparison failed its recall gate and does not ship; no cross-estate claim is made. | A record-shaped public conclusion and validation on an independent environment. |
