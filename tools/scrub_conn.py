...
connection states, relative timing, and the shape of the internal networks you
name on the command line.  The result is a traffic fingerprint, not an
anonymized or general-purpose export.

Name every private range the capture uses with a repeatable --source-internal.
Each one is remapped onto a documentation network, and a private address that
belongs to none of them stops the run rather than reaching the output.
"""
...
MAX_INTERNAL_NETWORKS = 254
EXTERNAL_CAPACITY = 762
DEFAULT_TOP_HOSTS = 30
UID_ALPHABET = string.ascii_letters + string.digits


def target_internal(count: int) -> tuple[ipaddress.IPv4Network, ...]:
    """Documentation networks receiving the source networks, in the same order."""
    return tuple(
        ipaddress.ip_network(f"192.168.{index + 1}.0/24") for index in range(count)
    )


def parse_source_internal(values: Sequence[str]) -> tuple[ipaddress.IPv4Network, ...]:
    """Read the internal ranges to remap, rejecting anything that cannot be one."""
    networks: list[ipaddress.IPv4Network] = []
    for value in values:
        try:
            network = ipaddress.ip_network(value, strict=False)
        except ValueError as exc:
            raise ScrubError(f"invalid source network {value}") from exc
        if not isinstance(network, ipaddress.IPv4Network):
            raise ScrubError(f"source network {network} is not IPv4")
        if not network.is_private:
            raise ScrubError(f"source network {network} is not a private range")
        networks.append(network)
    if not networks:
        raise ScrubError("name at least one source network")
    if len(networks) > MAX_INTERNAL_NETWORKS:
        raise ScrubError(f"at most {MAX_INTERNAL_NETWORKS} source networks are mappable")
    for first in range(len(networks)):
        for second in range(first + 1, len(networks)):
            if networks[first].overlaps(networks[second]):
                raise ScrubError(
                    f"source networks {networks[first]} and {networks[second]} overlap"
                )
    # The documentation ranges the output is written into report as private, so a
    # source network naming one of them would be remapped onto itself.
    for network in networks:
        for reserved in (*target_internal(len(networks)), *TARGET_EXTERNAL):
            if network.overlaps(reserved):
                raise ScrubError(
                    f"source network {network} overlaps the output range {reserved}"
                )
...
class Analysis:
    ...
    internal: tuple[set[ipaddress._BaseAddress], ...] = field(default_factory=tuple)
...
def _internal_index(
    ...
    source_internal: Sequence[ipaddress.IPv4Network],
) -> int | None:
...
def _classify_eligible_pair(
    ...
    source_internal = _internal_index(source, networks)
    destination_internal = _internal_index(destination, networks)
...
def analyze(path: Path, networks: Sequence[ipaddress.IPv4Network]) -> Analysis:
    # The loop below rebinds source_internal to an index, so the network list
    # keeps its own name.
    analysis = Analysis(internal=tuple(set() for _ in networks))
    for line_number, row in _rows(path):
        ...
        pair = _classify_eligible_pair(source, destination, line_number, networks)
        if pair is None:
            continue
...
def build_mappings(
    ...
    for sources, network in zip(analysis.internal, target_internal(len(analysis.internal))):
        mapping.update(
        ...
        with gzip.GzipFile(filename="", mode="wb", fileobj=raw_handle, mtime=0) as zipped:
            for line_number, row in _rows(source):
                ...
                pair = _classify_eligible_pair(
                    source_address, destination_address, line_number, networks
                )
...
def verify_output(source: Path, output: Path, internal_count: int) -> tuple[int, int]:
    ...
    for _, row in _rows(output):
        ...
        for address in addresses:
            sanctioned = (*target_internal(internal_count), *TARGET_EXTERNAL)
            if not any(address in network for network in sanctioned):
...
def scrub_file(
    ...
    *,
    ...
    analysis = analyze(source, source_internal)
    ...
    try:
        ...
        verified_rows, _ = verify_output(source, candidate, len(source_internal))
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        metavar="CIDR",
        help=(
            "a private network the capture uses, repeatable; name every one, "
            "for example --source-internal=10.1.0.0/24 --source-internal=10.2.0.0/24"
        ),
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        networks = parse_source_internal(args.source_internal)
        receipt = scrub_file(
            args.source,
            args.output,
            source_internal=networks,
    ...
    print(
        ...
        f"relative timing, and {mapped} internal network "
        f"{'shape' if mapped == 1 else 'shapes'}"
    )
